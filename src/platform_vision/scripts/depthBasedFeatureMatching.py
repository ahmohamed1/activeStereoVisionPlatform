...
class ComputeDepthBasedFeature:
    ...
    def findBestMatches(self,kp1,kp2,des1,des2):
    ...
    def findBestMatchingBFMatcher(self,kp1,kp2,des1,des2):
        bf = cv2.BFMatcher()
    ...
    def drawEpilinesAndMatches(self, imgL, imgR, pts1, pts2, kp1, kp2, good):
        ...
        img4_L = imgL
        img4_R = imgR
        img4_L,img4_R = self.drawlines(img4_L,img4_R,lines2,pts2,pts1)
    ...
    def rectify_shearing(self, H1, H2, imsize):
        ...
        ap = self.from_homg(H1.dot(a))
        bp = self.from_homg(H1.dot(b))
        cp = self.from_homg(H1.dot(c))
        dp = self.from_homg(H1.dot(d))
    ...
    def rectify_uncalibrated(self, lines1, lines2, x1, x2, F, imsize, threshold=5):
        """
    ...
    def undistortMap(self, H1, H2, img1, img2):
    ...
    def remap(self, img1, map1x, map1y, img2, map2x, map2y):
    ...
    def show_rectified_images(self, rimg1, rimg2):
    ...
    def rectifyWrapper(self, imgL, imgR, lines1, lines2, pts1, pts2, F):
        ...
        H1 = np.zeros((3,3))
        H2 = np.zeros((3,3))
        _, _, Q = self.rectify_uncalibrated(lines1, lines2, pts1, pts2, F, imgsize)
        # print ('H111 :' , H11)
        try:
            retval, H1, H2 = cv2.stereoRectifyUncalibrated(pts1, pts2, F, imgsize)
            # print ('H1 :' , retval)
        except cv2.error:
            print "cv2.error"
        ...
        # http://scicomp.stackexchange.com/questions/2844/shearing-and-hartleys-rectification
        S = self.rectify_shearing(H1, H2, imgsize)
        H1 = S.dot(H1)


        # Init Undistort, Map (mapx / mapy)
        img1, map1x, map1y, img2, map2x, map2y = self.undistortMap(H1, H2, imgL, imgR)

        # Remap
        rimg1, rimg2 = self.remap(img1, map1x, map1y, img2, map2x, map2y)

        return rimg1, rimg2, Q

    def disparityMapStereo(self, imgL,imgR):

        grayImageL = imgL.copy()
        grayImageR = imgR.copy()
        grayImageL = cv2.cvtColor(imgL, cv2.COLOR_BGR2GRAY)
        grayImageR = cv2.cvtColor(imgR, cv2.COLOR_BGR2GRAY)
        # BLOCK MATCHING ALGORITHM
        stereoBM = cv2.StereoBM_create(numDisparities=16, blockSize=9)
        disparityBM = stereoBM.compute(grayImageL,grayImageR)
        # cv2.imshow('disparityBM', disparityBM)
        # cv2.waitKey(0)
        return disparityBM
    ...
    def disparityMap(self, imgl, imgr):
        ...
        cv2.createTrackbar('NumDisparities','image',0,15,nothing)
    ...
    def disparityMapSGM(self, imgl, imgr):
        def nothing(x):
            pass
        imgL = imgl.copy()
        imgR = imgr.copy()
    ...
    def postfilterDisparity(self, imgl,imgr):
        import numpy as np
    ...
    def segmentImages(self, tr_img1, tr_img2):
        return tr_img1, tr_img2

    def registerImages(self, src_pts, dst_pts, src_img, dst_img):
    ...
    def mainLoop(self,ImageNameRight,ImageNameLeft):
        ...
        kp1, des1 = sift.detectAndCompute(surfImg_L,None)
        kp2, des2 = sift.detectAndCompute(surfImg_R,None)

        # Now we can match the keypoints
        # pts1, pts2, good = findBestMatches(kp1,kp2,des1,des2)
        pts1, pts2, good = self.findBestMatchingBFMatcher(kp1,kp2,des1,des2)
