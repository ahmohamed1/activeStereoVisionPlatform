...
def findBestMatchingBFMatcher(kp1,kp2,des1,des2):
    bf = cv2.BFMatcher()
...
def rectifyWrapper(imgL, imgR, lines1, lines2, pts1, pts2, F):
    ...
    H1 = np.zeros((3,3))
    H2 = np.zeros((3,3))
    _, _, Q = rectify_uncalibrated(lines1, lines2, pts1, pts2, F, imgsize)
    # print ('H111 :' , H11)
    try:
...
def postfilterDisparity(imgl,imgr):
    import numpy as np
    ...
    R, G, B, I = SMExtractRGBI(imgl)
    RG = ColourFeatureMap( R, G, B)
    RG = np.uint8(RG)
...
def generatePointCloud(rgb, depth, Q):
    #
...
if __name__ == "__main__":
    imgL = cv2.imread('data/TomatoDifferentBrightness/3left.jpg', 1)
    imgR = cv2.imread('data/TomatoDifferentBrightness/3right.jpg', 1)
    ...
    pts1, pts2, good = findBestMatchingBFMatcher(kp1,kp2,des1,des2)
    ...
    rimg1, rimg2, Q = rectifyWrapper(imgL, imgR, lines1, lines2, pts1, pts2, F)
    ...
    disp = postfilterDisparity(rimg1, rimg2)
    ...
    cv2.waitKey(0)
    # generatePointCloud(rimg1, pointCloud, Q)
